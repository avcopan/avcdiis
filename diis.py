...
class DIISManager(object):
    ...
    def add_entry(self, *array_error_pairs):
        ...
        Args:
            *array_error_pairs (list of tuples): Each tuple is a pair of lists
            of array-like objects.  The first one contains the arrays to be 
            extrapolated and the second one contains the corresponding
            residuals.
        """
    ...
    def extrapolate(self):
        ...
        arrays = tuple(
            sum(coeff * array for coeff, array in zip(coeffs, arrays))
    ...
    def _get_extrapolation_coefficients(self):
        ...
        Follows Pulay's `Improved SCF Convergence Acceleration`_.
        ...
        for i in range(n):
            for j in range(i, n):
                B[i, j] = sum(
                    np.vdot(error1.view(np.ndarray), error2.view(np.ndarray))
